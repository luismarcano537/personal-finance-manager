      ...
      <section className="grid w-full max-w-5xl overflow-hidden rounded-3xl border border-brand-border bg-white shadow-[0_24px_70px_rgba(31,41,51,0.08)] lg:grid-cols-[0.95fr_1.05fr]">
        <div className="hidden bg-brand-primary-soft p-10 lg:flex lg:flex-col lg:justify-between">
          <div>
            ...
            <h1 className="mt-4 max-w-sm text-4xl font-bold leading-tight text-brand-text-strong">
              Personal finance with a calmer monthly view.
            </h1>
            <p className="mt-5 max-w-md text-base leading-7 text-brand-text">
              Track income, expenses, balance, and transactions in one clean
              workspace built for everyday financial decisions.
            </p>
          </div>

          <div className="mt-10 rounded-3xl border border-brand-border bg-white/85 p-6 shadow-sm">
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-brand-gold-soft text-2xl font-bold text-brand-gold">
                $
              </div>
            </div>
            <div className="mt-6 h-2 overflow-hidden rounded-full bg-brand-surface-muted">
        ...
        <div className="px-5 py-8 sm:px-8 sm:py-10 lg:px-12">
          ...
          <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
            <div>
              ...
              <input
                autoComplete="email"
                className="mt-2 w-full rounded-2xl border border-brand-border bg-white px-4 py-3 text-sm text-brand-text-strong outline-none transition placeholder:text-brand-text-muted focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20"
                id="email"
                name="email"
            ...
            {error ? (
              <div className="rounded-2xl border border-[#FECACA] bg-brand-error-soft px-4 py-3">
                <p className="text-sm font-semibold text-brand-error">{error}</p>
              </div>
            ) : null}

            <button
              className="w-full rounded-2xl bg-brand-primary px-4 py-3 text-sm font-bold text-white shadow-[0_12px_30px_rgba(59,170,114,0.28)] transition hover:bg-brand-primary-dark focus:outline-none focus:ring-2 focus:ring-brand-primary focus:ring-offset-2 focus:ring-offset-white disabled:cursor-not-allowed disabled:bg-brand-border-strong disabled:text-brand-text-muted disabled:shadow-none"
              disabled={loading}
              type="submit"
            >
